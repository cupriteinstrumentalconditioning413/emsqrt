...
use emsqrt_io::arrow_convert::{
    emsqrt_to_arrow_schema, record_batch_to_row_batch, row_batch_to_record_batch,
};
...
fn test_record_batch_to_row_batch_all_types() {
    ...
    // Check string column
    assert_eq!(
        row_batch.columns[5].values[0],
        Scalar::Str("hello".to_string())
    );

    // Check binary column
    assert_eq!(
        row_batch.columns[6].values[0],
        Scalar::Bin(b"binary".to_vec())
    );
}

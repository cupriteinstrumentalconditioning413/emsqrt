...
fn test_budget_acquire_release() {
    ...
    // Acquire 100KB
    let guard = budget.try_acquire(100 * 1024, "test").expect("Acquire failed");
...
fn test_budget_exhaustion() {
    ...
    // Acquire 400KB
    let guard1 = budget.try_acquire(400 * 1024, "test").expect("First acquire failed");
    ...
    let guard2 = budget.try_acquire(200 * 1024, "test").expect("Acquire after release failed");
...
fn test_budget_guard_drop() {
    ...
    {
        let _guard1 = budget.try_acquire(100 * 1024, "test").expect("Acquire failed");
        assert_eq!(budget.used_bytes(), 100 * 1024);
        
        {
            let _guard2 = budget.try_acquire(200 * 1024, "test").expect("Acquire failed");
...
fn test_budget_concurrent_access() {
    ...
    let full_guard = budget.try_acquire(1024 * 1024, "test").expect("Should be able to acquire full budget");
...
fn test_budget_try_resize() {
    ...
    // Acquire 100KB
    let mut guard = budget.try_acquire(100 * 1024, "test").expect("Initial acquire failed");
    assert_eq!(budget.used_bytes(), 100 * 1024);
    assert_eq!(guard.bytes(), 100 * 1024);
...
fn test_budget_multiple_guards() {
    ...
    let guard1 = budget.try_acquire(100 * 1024, "test").expect("Acquire 1 failed");
    let guard2 = budget.try_acquire(200 * 1024, "test").expect("Acquire 2 failed");
    let guard3 = budget.try_acquire(300 * 1024, "test").expect("Acquire 3 failed");
...
fn test_budget_exact_capacity() {
    ...
    let guard = budget.try_acquire(1024, "test").expect("Should acquire exact capacity");
    assert_eq!(budget.used_bytes(), 1024);
    
    // No more room
    let result = budget.try_acquire(1, "test");
    assert!(result.is_none(), "Should not be able to acquire even 1 byte");
...
fn test_budget_fragmentation_handling() {
    ...
    let g4 = budget.try_acquire(300, "test").expect("Acquire 4 in freed space");
    assert_eq!(budget.used_bytes(), 900);

...
use emsqrt_operators::traits::{Operator, MemoryBudget};
use std::sync::{Arc, Mutex};
use test_data_gen::{create_temp_spill_dir, generate_random_batch, generate_sorted_batch};

fn setup_sort_operator(codec: Codec, spill_dir: String) -> (ExternalSort, Arc<Mutex<SpillManager>>) {
...
fn test_sort_in_memory() {
    ...
    let result = sort_op.eval_block(&[batch.clone()], &budget).expect("Sort failed");
    
    // Verify sorted
    assert_eq!(result.num_rows(), 6);
    assert!(verify_sorted(&result, "sort_key"), "Result should be sorted");
...
fn test_sort_with_spill() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Sort with spill failed");
...
fn test_sort_multiple_runs() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Multi-run sort failed");
...
fn test_sort_large_keys() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Sort with large keys failed");
...
fn test_sort_empty_batch() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Sort empty batch failed");
...
fn test_sort_with_nulls() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Sort with nulls failed");
...
fn test_sort_already_sorted() {
    ...
    let result = sort_op.eval_block(&[batch], &budget).expect("Sort already sorted failed");

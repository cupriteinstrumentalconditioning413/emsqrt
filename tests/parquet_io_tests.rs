...
fn test_parquet_compression() {
    ...
    let mut reader =
        ParquetReader::from_path(&parquet_file, None, 1000).expect("Failed to create reader");
    let read_batch = reader
        .next_batch()
        .expect("Failed to read")
        .expect("Expected batch");
...
fn test_parquet_empty_batch() {
    ...
    let read_batch_opt = reader.next_batch().expect("Failed to read");
...
fn test_parquet_multi_batch_read() {
    ...
    let large_batch = RowBatch {
        columns: vec![
            ...
            Column {
                ...
                values: (0..1000)
                    .map(|i| Scalar::Str(format!("value{}", i)))
                    .collect(),
            },
        ],
    };
    ...
    let mut reader =
        ParquetReader::from_path(&parquet_file, None, 100).expect("Failed to create reader");
